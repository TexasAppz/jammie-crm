...
function Form1003({ loan, onBack, showToast, onLoanUpdated }) {
      ...
      .then(existing => {
        if (existing && existing.id) {
          setBorrowers([{
            ...
            addrSameAs:    existing.addr_same_as    || '',
            isVeteran:     existing.is_veteran === 1,
            isDisabledVet: existing.is_disabled_vet === 1,
            isExempt:      existing.is_exempt_funding_fee === 1,
            vaUseType:     existing.va_use_type     || '',
            mailingSame:   existing.mailing_same_as_present === 1,
            mailingAddr1:  existing.mailing_address_street || '',
            mailingUnit:   existing.mailing_address_unit_num || '',
            mailingCity:   existing.mailing_address_city  || '',
            mailingState:  existing.mailing_address_state || '',
            mailingZip:    existing.mailing_address_zip   || '',
            mailingCountry:existing.mailing_address_country || 'United States',
          ...
          setFormData(p => {
            ...
            // BUG FIX: `!incomes` is false when incomes_json was saved as
            // '[]' (happens the instant any autoSave fires before income is
            // entered — e.g. right after creating a loan, before a MISMO
            // import runs). An empty array is truthy in JS, so the correct
            // gross_income_monthly_* values were being silently ignored.
            if ((!incomes || incomes.length === 0) && (existing.employee_or_business_nm || parseFloat(existing.gross_income_monthly_base) > 0)) {
              incomes = [{
            ...
            return {
              ...
              reos:        reos        || p.reos,
              mortPres: existing.he_mortgage_pres ?? p.mortPres,
              mortProp: existing.he_mortgage_prop ?? p.mortProp,
              otherLoans: existing.he_other_loans_pres ?? p.otherLoans,
              otherBPres: existing.he_other_loans_prop ?? p.otherBPres,
              hazPres: existing.he_hazard_pres ?? p.hazPres,
              hazProp: existing.he_hazard_prop ?? p.hazProp,
              taxPres: existing.he_taxes_pres ?? p.taxPres,
              taxProp: existing.he_taxes_prop ?? p.taxProp,
              miPres: existing.he_mi_pres ?? p.miPres,
              miProp: existing.he_mi_prop ?? p.miProp,
              hoaPres: existing.he_hoa_pres ?? p.hoaPres,
              hoaProp: existing.he_hoa_prop ?? p.hoaProp,
              floodPres: existing.he_flood_pres ?? p.floodPres,
              floodProp: existing.he_flood_prop ?? p.floodProp,
              otherPres: existing.he_other_pres ?? p.otherPres,
              otherProp: existing.he_other_prop ?? p.otherProp,
              rentPres: existing.he_rent_pres ?? p.rentPres,
              land: existing.dot_land ?? p.land,
              improvements: existing.dot_improvements ?? p.improvements,
              refiBalance: existing.dot_refi_balance ?? p.refiBalance,
              ccDebts: existing.dot_cc_debts ?? p.ccDebts,
              closingCosts: existing.dot_closing_costs ?? p.closingCosts,
              discount: existing.dot_discount ?? p.discount,
              financedFees: existing.dot_financed_fees ?? p.financedFees,
              otherCredits: existing.dot_other_credits ?? p.otherCredits,
              otherBProp: existing.dot_other_loans ?? p.otherBProp,
  ...
  const calcDTI = () => {
    ...
    const otherDebts = (formData.liabilities||[]).filter(l=>l.dti==='Include' && !l.paid_off)
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        tax_same_as:         b.taxSameAs    || null,
        addr_same_as:        b.addrSameAs   || null,
        is_veteran:          b.isVeteran ? 1 : 0,
        is_disabled_vet:     b.isDisabledVet ? 1 : 0,
        is_exempt_funding_fee: b.isExempt ? 1 : 0,
        va_use_type:         b.vaUseType    || null,
        mailing_same_as_present: b.mailingSame ? 1 : 0,
        mailing_address_street:  b.mailingAddr1  || null,
        mailing_address_unit_num:b.mailingUnit   || null,
        mailing_address_city:    b.mailingCity   || null,
        mailing_address_state:   b.mailingState  || null,
        mailing_address_zip:     b.mailingZip    || null,
        mailing_address_country: b.mailingCountry|| null,
        prev_addresses_json: JSON.stringify(b.prevAddresses || []),
        he_mortgage_pres: parseFloat(formData.mortPres) || null,
        he_mortgage_prop: parseFloat(formData.mortProp) || null,
        he_other_loans_pres: parseFloat(formData.otherLoans) || null,
        he_other_loans_prop: parseFloat(formData.otherBPres) || null,
        he_hazard_pres: parseFloat(formData.hazPres) || null,
        he_hazard_prop: parseFloat(formData.hazProp) || null,
        he_taxes_pres: parseFloat(formData.taxPres) || null,
        he_taxes_prop: parseFloat(formData.taxProp) || null,
        he_mi_pres: parseFloat(formData.miPres) || null,
        he_mi_prop: parseFloat(formData.miProp) || null,
        he_hoa_pres: parseFloat(formData.hoaPres) || null,
        he_hoa_prop: parseFloat(formData.hoaProp) || null,
        he_flood_pres: parseFloat(formData.floodPres) || null,
        he_flood_prop: parseFloat(formData.floodProp) || null,
        he_other_pres: parseFloat(formData.otherPres) || null,
        he_other_prop: parseFloat(formData.otherProp) || null,
        he_rent_pres: parseFloat(formData.rentPres) || null,
        dot_land: parseFloat(formData.land) || null,
        dot_improvements: parseFloat(formData.improvements) || null,
        dot_refi_balance: parseFloat(formData.refiBalance) || null,
        dot_cc_debts: parseFloat(formData.ccDebts) || null,
        dot_closing_costs: parseFloat(formData.closingCosts) || null,
        dot_discount: parseFloat(formData.discount) || null,
        dot_financed_fees: parseFloat(formData.financedFees) || null,
        dot_other_credits: parseFloat(formData.otherCredits) || null,
        dot_other_loans: parseFloat(formData.otherBProp) || null,
  ...
  const autoSave = async () => {
    ...
    try {
      ...
      if (loan.id) {
        ...
        const existing = await db.form1003.getByLoan(loan.id).catch(()=>null);
        if (existing?.id) {
          // Do NOT swallow this. A failed save here previously looked like
          // success and the data vanished on reload.
          await db.form1003.update(existing.id, payload)
      ...
      // went unnoticed and data silently reverted on reload.
          ...
          <div style={{marginBottom:28}}>
            <div style={{fontSize:16,fontWeight:700,color:'var(--text)',marginBottom:4}}>
              Monthly Liability: <span style={{color:'#b91c1c'}}>
                ${(formData.liabilities||[]).filter(l=>l.dti==='Include' && !l.paid_off).reduce((a,x)=>a+(parseFloat(x.payment)||0),0).toLocaleString('en-US',{minimumFractionDigits:2})}
              </span>
